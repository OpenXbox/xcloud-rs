...
use gilrs::{
    Gilrs, Event,
    ff::{BaseEffect, EffectBuilder}
};

struct GStreamerWebrtcClient {
...
impl GStreamerWebrtcClient {
    ...
    /// Initialize WebRTC connection parameters
    ...
    fn create_gstreamer_pipeline(&self) -> Result<(gst::Pipeline, gst_app::AppSrc, gst_app::AppSrc), Box<dyn std::error::Error>> {
        ...
        Ok((pipeline, appsrc_audio, appsrc_video))
    }

    async fn setup_ice_candidate_handling(&self) -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn handle_sdp_exchange(&self) -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn open_data_channels() -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn setup_expected_streams() -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn start_receiving_from_channels() -> Result<JoinHandle<!>, Box<dyn std::error::Error>> {
    ...
    async fn setup_incoming_tracks_handling() {
            ...
            if let Some(track) = track {
                ...
                Box::pin(async move {
                    ...
                    if mime_type == MIME_TYPE_OPUS.to_lowercase() {
                        ...
                        tokio::spawn(async move {
                            loop {
                                let buffer = Self::read_packet_into_gst_buf(&track).await.expect("Failed to read packet into buffer");
    ...
    async fn start(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let (pipeline, appsrc_audio, appsrc_video) = self.create_gstreamer_pipeline()?;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let client = GStreamerWebrtcClient {};
    client.start().await?;
